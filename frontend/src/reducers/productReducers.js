...
export const productUpdateReducer = (state = { product: {} }, action) => {
	switch (action.type) {
		case c.PRODUCT_UPDATE_REQUEST:
			return {
				loading: true,
			};
		case c.PRODUCT_UPDATE_SUCCESS:
			return {
				loading: false,
				success: true,
				product: action.payload,
			};
		case c.PRODUCT_UPDATE_FAIL:
			return {
				loading: false,
				error: action.payload,
			};
		case c.PRODUCT_UPDATE_RESET:
			return { product: {} };
		default:
			return state;
	}
...
export const productTopRatedReducer = (state = { products: [] }, action) => {
	switch (action.type) {
		case c.PRODUCT_TOP_REQUEST:
			return {
				loading: true,
				products: [],
			};
		case c.PRODUCT_TOP_SUCCESS:
			return {
				loading: false,
				products: action.payload,
			};
		case c.PRODUCT_TOP_FAIL:
			return {
				loading: false,
				error: action.payload,
			};
